...
def choice(request, element_id, session_id):
    ...
    # if element_id == X    OR X+1 OR X+2 OR X+3 (4 seed types)
    seed_search_choices = ['55', '58', '61', '64']
    if element_id in seed_search_choices:
        ...
        for wav in wavs:
            ...
            # 0 bags 1 seedtype 2 phonenr 3 datetime
            if wav_data[1] == seedtype: # request is searching for this
                print(wav, wav_data[1], wav_data[2])
                new_voice_labels.append(url + wav)
                phones.append(wav_data[2])

        context['phone_nrs'] = phones
        context['post_url'] = (context['choice_options_redirect_urls'])[0] #'/vxml/message/12/39'

        # 3. update the context to match the request
